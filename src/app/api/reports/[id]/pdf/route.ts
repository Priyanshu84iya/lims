...
function renderAiSection(analysis: AiAnalysis) {
  const finding = (f: { parameter: string; value: string; unit: string; reference: string; status: string; note?: string }, abnormal: boolean, key: string) => `
  ...
  const abnormalBody = analysis.abnormalFindings.length
    ? analysis.abnormalFindings.map((f) => finding(f, true, "abn")).join("")
  ...
  const normalBody = analysis.normalFindings.length
    ? analysis.normalFindings.map((f) => finding(f, false, "nrm")).join("")
    : "";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      if (typeof body?.analysis === "string" && body.analysis.trim()) analysis = body.analysis;
    } catch {
      // no body provided — render without the AI section
    }
    const report = await db.orm.public.Report.where({ id: Number(id), labId: auth.labId }).first();
    if (!report) {
